...
def command_launch_remote_debugger(_connection=None):
    try:
        from plumbbuddy_proxy.api import gateway, BridgedUiNotFoundError
        look_up_bridged_ui_eventually = gateway.look_up_bridged_ui(_bridged_ui_uuid)

        def handle_look_up_result(bridged_ui):
            output = CheatOutput(_connection)
            output("The bridged UI for the REPL is already open in PlumbBuddy.")
            bridged_ui.focus()

        def handle_look_up_fault(fault):
            if not isinstance(fault, BridgedUiNotFoundError):
                output = CheatOutput(_connection)
                output(f"I tried to launch the REPL bridged UI but it failed for this reason: {str(fault)}")
                return
            ...
            request_bridged_ui_eventually.then(handle_request_result) # tell the Eventual what to do when PB answers
